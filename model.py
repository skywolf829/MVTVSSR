...
def spatial_derivative2D(field, axis):
    if(axis == "x"):
        weights = torch.tensor([[0, 0, 0], [-1/2, 0, 1/2], [0, 0, 0]])
        weights = np2torch(weights.view(1, 1, 3, 3))
        output = F.conv2d(field, weights, padding=1)
    elif(axis == "y"):
        weights = torch.tensor([[0, -1/2, 0], [0, 0, 0], [0, 1/2, 0]])
        weights = np2torch(weights.view(1, 1, 3, 3))
        output = F.conv2d(field, weights, padding=1)
    return output
...
def generate(generators, opt, minibatch, starting_frames, device):
    ...
    for i in range(len(generators)):
        generated_image = F.interpolate(generated_image, size=self.resolution, mode=opt["upsample_mode"])
...
def train_single_scale(process_num, generators, discriminators_s, opt):
    ...
    for epoch in range(opt["iteration_number"], opt["epochs"]):
        for i, (output_dataframes) in enumerate(dataloader):
            ...
            F.interpolate(downscaled_dataframes, size=opt["scales"][len(generators)+1], mode=opt["upsample_mode"])
