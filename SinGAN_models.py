...
def train_single_scale(generators, discriminators, opt):
    ...
    for epoch in range(opt['iteration_number'], opt["epochs"]):
        ...
        writer.add_scalar('Gradient_loss/%i'%len(generators), gradient_loss / opt['alpha_5'], epoch)
        writer.add_scalar('TAD/%i'%len(generators), phys_loss / opt["alpha_3"], epoch)
        writer.add_scalar('path_loss/%i'%len(generators), path_loss / opt['alpha_6'], epoch)
