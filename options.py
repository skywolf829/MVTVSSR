...
class Options():
    def get_default():
        ...
        opt["data_folder"]             = "CM1_3D/train"
        opt["validation_folder"]       = "CM1_3D/validation"
        ...
        opt["temporal_downscale_ratio"]= 0.5       # Temporal downscale ratio between levels
        opt["temporal_downscale_mode"] = "linear"  # How to do the temporal downscaling
        opt["min_dimension_size"]      = 20        # Smallest a dimension can go as the smallest level
        ...
        opt["base_resolution"]         = [40, 128, 128]# Base resolution for full scale images [rows, cols] aka [y, x] aka [height, width]
