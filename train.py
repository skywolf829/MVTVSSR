    ...
    for i in range(opt["n"]-1):
        ...
        if(opt["train_distributed"]):
            mp.spawn(
                ...
                args=(generators, discriminators_s, opt)
            )
            opt = load_options(os.path.join(save_folder, opt["save_name"]))
            generators, discriminator_s = load_models(opt, "cpu")
        else:
            generator, discriminator_s = train_single_scale(0, generators, discriminators_s, opt)
